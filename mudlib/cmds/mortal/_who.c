...
string list_users(string *races, string order) {
    object *who, *tmp;
    int max;
    int num, i, j, total, header_num;
    string type_str, who_list, line;
	string *headers;
	int invis_admin_count = sizeof(filter_array(users(), "invis_admin", this_object(), races));
																								   

    who = filter_array(users(), "which_users", this_object(), races);
    total = sizeof(who);
    headers=get_dir("/news/who_headers/");
    header_num=random(sizeof(headers));
    who_list = read_file("/news/who_headers/"+headers[header_num]);
    //else
...
//who_list += "Number of users: " +(sizeof(users()) - invis_admin_count) +" (" +total+" shown)\n";	
//TLNY2021 Removed
//who_list += "Number of users: " +(sizeof(users())) +" (" +total+" shown)\n";
who_list += "Number of users: " +total+"\n";		
...
int invis_admin(object who, string *races) {
    string my_race;
    int i;
...
/* //TLNY2024 Examples of what was tried to get invis for player only to show up on who
 if (who->query_invis())
    return 1;
    if (who->query_invis() && !archp(this_player()))
    return 0;

if (who->query_invis() && (!archp(who) || archp(this_player())))
    return 0;

if (who->query_invis() && (!this_player() || !archp(this_player())))
    return 0;
