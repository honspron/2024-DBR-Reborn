//  A join room for assassins, well commented and intended fo use as
...
/*  
    set_related_guilds(({ "thief", "nightblade", }));
    //Other:      "fighter","tinker", "chaotic-lord", "sword-mage"
    set_odd_guilds(({ "elementalist", "enchanter", "elemental-mage" }));
    set_hated_guilds(({ "arch-mage" }));
    set_no_guilds(({ "nethermancer", "dark-paladin", "necromancer", "cleric", "war-priest", "paladin" }));
*/
	
  set("short", "The Assassin's Guild Joining Room");
  set("long", 
  "This is the anteroom of the dreaded Assassin Guild.  The shadows "
  ...
  set_exits( ([ 
		"south" : "/d/daybreak/room/db_slum",
        "north" : "/d/damned/guilds/assassin/rooms/foyer_room" ]) );

  set("guildmaster", "/d/damned/guilds/assassin/assassin_gm");
  //  The guildmaster is automatically cloned if no pc guildmaster is
  //  found.

  set_guild_obj("/d/damned/guilds/assassin/assassin_obj");
...
int bounty(string str) {
  ...
  if( (file_size(who + ".o") < 0) && (nom != "all") ) 
    return notify_fail("The guild has never heard of that person.\n");
        if("/d/damned/guilds/join_rooms/assassin_join"->is_member(who)) return notify_fail("The guild notes your attempt to place"
