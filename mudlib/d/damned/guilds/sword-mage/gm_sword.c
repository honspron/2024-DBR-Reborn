...
object ob;
   ...
   if(this_player()->is_player() && (int)this_player()->query_level() < 16) {
     write("%^BOLD%^Only sword-mages of level 16 or higher may use this item.");
    return 0;
}

   if((string)this_player()->query_class() != "sword-mage") {
      write("%^BOLD%^Only sword-mages may use this item.");
    return 0;
  }
