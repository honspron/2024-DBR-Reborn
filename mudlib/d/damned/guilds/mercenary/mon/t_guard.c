...
int block_exit() {
  ...
  object join_room, *inv;
	...
	if(find_player(p_name) && find_player(p_name)->query_class() == "mercenary") return 0;
	     force_me("speak common");
     force_me("say I will not allow a pet named "+TP->query_name()+" from entering as you are not a Mercenary");		
		return 1;
    }
...
force_me("speak common");
     force_me("say Only Mercenarys may pass.");
    return 1;
  }

 join_room = find_object_or_load("/d/damned/guilds/join_rooms/mercenary_join");
