...
int make_me_gm() {
 ...
 if(this_player()->is_player() && (int)this_player()->query_level() < 16) {
     write("%^BOLD%^Only Assassins of level 16 or higher may use this item.");
    return 0;
}

   if((string)this_player()->query_class() != "assassin") {
      write("%^BOLD%^Only Assassins may use this item.");
    return 0;
  }
  ...
  seteuid(getuid());
  ob = new("/d/damned/guilds/assassin/assassin_obj");
