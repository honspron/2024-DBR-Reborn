...
int block_exit() {
  ...
  object join_room, *inv;
	...
	if(find_player(p_name) && find_player(p_name)->query_class() == "nethermancer") return 0;
	     force_me("speak common");
     force_me("say I will not allow a pet named "+TP->query_name()+" from entering as you are not a Nethermancer");		
		return 1;
    };
  ...
  if(wizardp(this_player())) return 0;
   if((string)this_player()->query_class() != "nethermancer") {
	     force_me("speak common");
     force_me("say Only nethermancers may pass.");
    return 1;
  }

 join_room = find_object_or_load("/d/damned/guilds/join_rooms/nethermancer_join");
