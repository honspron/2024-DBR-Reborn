...
set_property("caster message", "You cast %^BOLD%^%^BLACK%^shadow edge %^RESET%^at $T!");
set_property("target message", "");
set_property("observer message", "$C casts %^BOLD%^%^BLACK%^shadow edge %^RESET%^at $T.");
...
if(check_stack(at, ob)) {
	...
	ob->set_extra_wc(([ "unholy" : 3*power ]));
ob->start_shadow(at, props["duration"], "%^BOLD%^%^BLACK%^A shadow edge spell wears off.%^RESET%^");
  }    
	else{	
