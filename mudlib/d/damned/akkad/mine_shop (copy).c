...
void reset() {
  ::reset();
  if(!present("goff"))
    new("/d/damned/akkad/obj/mon/goff")->move(this_object());
  if(!present("key rack"))
    new("/d/damned/akkad/obj/misc/mine_key_rack")->move(this_object());
  return;
}
