...
void create() {
    ...
    set("long",
      ...
      "Directions: 1) Read the race help... <help races>.\n"
      "            2) Pick the race... pick <race>.\n \n"
...
void do_rolls() {
  ...
  "points.  You will be provided with a table of stat costs.\n"
    "By the way, if you try to raise a stat above 110 this does not include racial bonuses, it will wrap to 35!\n");
...
if(strlen(str) >= 2 && str[0..1] == "ch") m = 2;
  ...
  write("\n"+
      ...
      "If you are a new player and have no idea what to do I recommend having at laest %^YELLOW%^90 CON 90 STR and 80 DEX%^RESET%^ base stats to start regardless of racial bonuses or minuses this means to put in 75 points to each of those stats");
...
//TLNY2020 added to get past first 3 new levels if people choose.
// "old exp", 50149 <-- Level 5
// "old exp", 10149 <-- Level 3
    who->set_property("old exp", 10149);
	while(i--) {
  ...
  if(dev_tot < 55)
      message("info", "Your stats suck.  If you would like to be deleted, "+
        "ask an Arch immediately! or type suicide and remake", who);
  who->set_property("dev points",dev_tot);
  who->set_property("dev point base",dev_tot);
  who->set_max_hp(6 * ((int)this_player()->query_stats("constitution") + 20) / 5);
  who->augment_body((int)this_player()->query_stats("constitution"));
  who->set_max_mp(6 * ((int)this_player()->query_stats("intelligence") + 20) / 5);
...
void set_ansi() {
    ...
    write("\nOne more thing...\n%^BOLD%^%^RED%^do you want to play in xterm-256 color (y or n)?%^RESET%^");
...
void set_ansi_two(string str) {
    ...
    if(str == "n") {
	write("Ok.  Terminal set to \"unknown.\"  See 'help setenv' for more info. The first thing you should do is type 'look' and then follow directions into the castle make sure to 'read sign' in each room");
	...
	write("Terminal set to xterm-256 colour. The first thing you should do is type %^YELLOW%^'look'%^RESET%^ and then follow directions make sure to %^YELLOW%^'read sign'%^RESET%^ in each room");
          this_player()->move("/d/newbieville/rooms/newbie0");
        if(wizardp(this_player()) || this_player()->query_property("reincarnate")) {
          write("You are done.");
          return;
        }
