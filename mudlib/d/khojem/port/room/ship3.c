...
void create() {
    ...
    add_exit("/d/khojem/port/room/s25","west");
    add_exit("/d/khojem/port/room/ship3a","down");
    add_invis_exit("west");
    set_door("hatch2","/d/khojem/port/room/s25","west",0);
    set_func("hatch2","open","do_open");
    set_func("hatch2","close","do_close");
    set_items(([
     ({ "hatch2" }) : 
...
void look_at_door() {
  ...
  say(this_player()->query_cap_name()+" examines a hatch2.\n");
}

int do_open() {
  remove_invis_exit("west");
  "/d/khojem/port/room/s25"->remove_invis_exit("east");
  return 1;
}
 
int do_close() {
  add_invis_exit("west");
  "/d/khojem/port/room/s25"->add_invis_exit("east");
  return 1;
}
