...
void reset() {
	...
	for(i=1; i<=3;i++) {
		new(MAGIC+"wolf_orb")->move(this_object());
		new(MAGIC+"brownie_orb")->move(this_object());
		new(MAGIC+"dire_wolf_orb")->move(this_object());
		if(i == 1) { new(MAGIC+"minotaur_orb")->move(this_object()); }
	}
	return;
}
