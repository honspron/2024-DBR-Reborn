...
void create() {
  ::create(); 
	...
	"watch your step here and pay your bills on time, or you'll have to "
	"deal with him.");
    set_level(18);
    set_hp(6000);
	set("gang bang", 0);
...
void directed_message(object who, string str) {
  ...
  if(lower_case(replace_string(str, " ","")) == "help") {
    say_this("This is how you rent a room.", who);
...
// KURIL DATA FILE
			     ...
			     sec);
// KURIL DATA FILE
    room_obj = find_object(sprintf("/d/nocte/kuril/virtual/inn_%d.inn", room_num));
    if(room_obj)
