...
void create() {
	...
	set_short("A crystal orb with a small brownie inside");
	set("long",
...
int shatter_orb(string str) {
	...
	if(str == "orb") {
		message("info", "With a loud POP!!!, the orb shatters into "
			...
			"emerges a brownie which immediately walks up to its "
			"new master.", this_player());
		message("info", (string)this_player()->query_cap_name()+
			" shatters a small crystal orb!  A brownie emerges from "
