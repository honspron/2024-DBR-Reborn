...
if(!present("insectoid_queen01")) {
         new(HIV_MON+"insectoid_queen01")->move(this_object());
  }  
}
...
void create() {
    ...
    set("long",
	"You climb past the rocks and underbrush to reach a part of "
              "the hill which has been cleared of its cover.  It appears to "
              "be the most heavily defended part of the hill.  You rapidly "
              "realize that it is the temporary site of the insectoids royal "
              "chamber.  You dont think they are going to just let you "
              "have an audience with their queen.\n" );
    set("night long",
    ...
    ({ "rock", "rocks" }) :
        "They have been moved out of the immediate area, providing a "
        "small clearing.  The rocks to the east are large enough to block "
     ...
     ({ "groundcover", "undergrowth"  }) :
        "The brush growing all over the hill has been cleared away in "
        "the immediately area.  Outside the cleared circle, however, it is "
        "as heavy as ever.",
]));
    set_exits((["south":HIV_ROOMS+"village_98.c",
                     "southwest":HIV_ROOMS+"village_97.c",
                     "northwest":HIV_ROOMS+"village_95.c",
                     "west":HIV_ROOMS+"village_99.c",
                     "north":HIV_ROOMS+"village_90.c"])); 
}
