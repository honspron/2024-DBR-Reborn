...
void reset(){
  ::reset();
  if(!present("insectoid_iw01")) {
         new(HIV_MON+"insectoid_iw01")->move(this_object());
         new(HIV_MON+"insectoid_iw01")->move(this_object());
         new(HIV_MON+"insectoid_iw01")->move(this_object());
         new(HIV_MON+"insectoid_iw01")->move(this_object());
         new(HIV_MON+"insectoid_iw01")->move(this_object());
...
void create() {
    ...
    set("long",
	"You have risen high enough on the hill to see over the "
              "surrounding forest.  Unfortunately, the hill itself is still "
              "covered with rocks and underbrush.  You have, however, "
              "come out on to a point on the hill that can only be "
              "described as in infirmary of some type.  You wouldnt "
              "have expected the insectoids to care for their wounded.  "
              "Nor have you seen any sign of what could have hurt them "
              "in the first place.\n" );
    set("night long",
    ...
    set_exits((["southeast":HIV_ROOMS+"village_99.c",
                     "west":HIV_ROOMS+"village_77.c",
                     "east":HIV_ROOMS+"village_95.c",
                     "south":HIV_ROOMS+"village_93.c",
                     "northeast":HIV_ROOMS+"village_79.c",
                     "north":HIV_ROOMS+"village_78.c"])); 
}
