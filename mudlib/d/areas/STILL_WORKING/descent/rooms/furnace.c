...
int smelt(string str) {
   ...
   amt = to_int(percent(150, (int)TP->query_skill("blacksmith"))+random(50));
...
int mint(string str) {
   ...
   message("info", (string)this_player()->query_cap_name() +
          " mints some "+(string)mat+" coins.", environment(this_player()),
            ({ this_player() }));
   amt = to_int(percent((int)TP->query_skill("blacksmith"), 100)) + 10;
...
"In the east wall is an arched doorway back to the forge room.");
    set_items(
