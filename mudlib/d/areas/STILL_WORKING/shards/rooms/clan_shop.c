...
void reset() {
    ::reset();
    if(!present("sphere"))
	new(OBJ+"clan_crystal")->move(this_object());
}
...
void create() {
    ...
    add_exit("/d/areas/shards/rooms/ak_tavern1", "east");
    set("long",
      "The walls and ceiling of this building have mostly collapsed, "
      "and only the northern wall remains standing. On it, signs "
      "rest, untouched by the ages and destruction.");
...
int buy_item(string str) {
	...
	*/
	...
	if(!ob) {
	    write("You have no guard to upgrade.\n");
	    return 1;
	}
	ITEM_BANK_D->save_items(ob, "clans", this_player()->query_guild() + "_guard");
	inv = all_inventory(ob);
	for(i=0;i<sizeof(inv);i++) inv[i]->remove();
	ob->remove();
	...
	write("You upgrade one of your guards.\n");
	return 1;
    }
