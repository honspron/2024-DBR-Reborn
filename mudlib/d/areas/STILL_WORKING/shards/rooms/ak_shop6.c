...
void create() {
   ...
   add_exit(ROOMS+"ak_markse", "west");
   add_exit(STORE+"shop6_store", "north");
   add_exit(STORE+"locksmith", "south");
   set_door("wooden door", STORE+"shop6_store", "north",
	    "ak shop6 store");
   set_open("wooden door", 0);
   set_locked("wooden door", 1);
   open = 1;
   set_door("east door", ROOMS+"ak_markse", "west",
	    "ak shop6");
...
void open_door() {
  set_locked("east door", 0);
  set_open("east door", 1);
  call_other(ROOMS+"ak_markse", "set_locked",
	     "east door", 0);
  call_other(ROOMS+"ak_markse", "set_open",
	     "east door", 1);
  open = 1;
  this_object()->reset();
  return;
...
void close_door() {
  ...
  while(i--) {
    if(living(inv[i])) {
      ...
      inv[i]->move(find_object_or_load(ROOMS+"ak_markse"));
    }
  }
