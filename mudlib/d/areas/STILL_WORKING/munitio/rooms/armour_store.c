...
#define LEG_ARMOUR ({ "chain-greaves", "plate-greaves", "scale-greaves", "leather-pants", "studded-leather-pants" })
...
void reset() {
  ...
  bob = present("shopkeeper", 
                find_object_or_load(ROOMS+"armour_shop"));
  if(bob) {
    message("info", "Feika says in Common: Wait a second.  I've got a new "+
...
void finish_deliver(object bob) {
  if(bob)
    message("info", "Feika says in Common: It's all here now.",
	    environment(bob));
  return;
}
