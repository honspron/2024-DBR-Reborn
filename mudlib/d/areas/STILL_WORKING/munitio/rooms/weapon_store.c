#include <std.h>
#include "../munitio.h"
#define WEAPONS ({ "broad-sword", "rapier", "knife", \
  "stiletto", "military-pick", "scimitar", "battle-axe", "military-fork", \
  "halberd", "trident", "spear", "hammer", \
  "war-hammer", "baton" })
...
void reset() {
  ...
  bob = present("shopkeeper", 
                find_object_or_load(ROOMS+"weapon_shop"));
  if(bob) {
    message("info", "Marko says in Common: Wait a second.  I've got a new "+
...
void remove_all(object *inv) {
  ...
  i = sizeof(inv);
  while(i--) if(inv[i] && environment(inv[i]) == this_object()) inv[i]->remove();
  return;
}

void finish_deliver(object bob) {
  if(bob)
    message("info", "Marko says in Common: It's all here now.",
	    environment(bob));
  return;
}
