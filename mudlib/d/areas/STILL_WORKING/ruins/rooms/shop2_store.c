...
void create() {
  ...
  set_long("This is the storage room for Opis's Armour Store. Lots of
valuable looking wares lie about.");
  ...
  add_exit((ROOMS+"shop2"), "west");
  set_door("iron door", (ROOMS+"shop2"), "west",
           "fighter store key");
...
void reset() {
  ...
  bob = present("shopkeeper", 
                find_object_or_load(ROOMS+"shop2"));
  if(bob) {
    message("info", "Opis says in Common: Wait a second.  I've got a new
"+
...
void finish_deliver(object bob) {
  if(bob)
    message("info", "Opis says in Common: It's all here now.",
            environment(bob));
  return;
}
