...
void create() {
   ...
   set_melee_damage(([ "crushing" : zlevel*zskill*lvl*llvl, damage1 : zlevel*zskill*lvl*llvl, damage2 : zlevel*zskill*lvl*llvl ]));


   set_money("gold", random(zlevel*lvl)+zlevel); 

set_name("WildBoyz");
   set("id", ({ "wildboyz", "orc", "ork", "wildboy", "feral ork" }));

 set("long",@ext A Wildboy Ork raiding party will often come across a small community of Orks that have spored away from the larger tribes. These Orks are usually armed with the most basic, primitive weapons.
ext);


    switch(random(2)){
      case 0:set_short("["+zlevel+"]Wildboyz");break;
      case 1:set_short("["+zlevel+"]Feral Ork");break;
}
...
object ob;
   ...
   message("info", "%^GREEN%^%^BOLD%^Before the Ork's death, a dwarf appears to "
...
switch(random(2)){
    case 1:
message("info","%^COLOR200%^Very Rare loot drops on the ground and get it%^RESET%^", environment(this_object()), this_object());
