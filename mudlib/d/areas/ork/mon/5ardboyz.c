...
set_name("Ardboyz");
   set("id", ({ "ardboyz", "orc", "ork", "eavy armour" }));

 set("long",@ext 'Ardboyz are the heavily-armoured standard Ork infantry who fulfill the same tactical role as the basic Slugga Boyz but can take a great deal more punishment. 'Aedboyz outfit themselves in thick scrap metal plates that they've scavenged and that is known as 'Eavy Armour. Despite poor maintenance, the result is a suit of armour than can withstand most small arms fire. 
ext);

    switch(random(2)){
      case 0:set_short("["+zlevel+"]Ardboyz");break;
      case 1:set_short("["+zlevel+"]Eavy Armour");break;
}
