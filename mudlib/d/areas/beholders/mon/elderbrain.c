...
set_name("Elder Brain");
   set("id", ({"elder brain", "elder", "brain", "Elder Brain" }) );
   set("long",@ext A Elder Brain
ext);
   set_short("["+zlevel+"]Elder Brain");    
   ...
   set_hp(zlevel*450 + zlevel*2);
   set_max_hp(zlevel*450 + zlevel*2);
//set_hp(100);
//set_max_hp(100);
    //set_max_mp(7000);
    //set_mp(7000);
...
*/
    ...
    add_spell("syphon soul", "$A"); //Lifesteal++
    set_spell_level("syphon soul", 6);
    
    set_skill("necromancy", zspell*zlevel);
    add_spell("dark breath", "$A"); //take Half HP
    set_spell_level("dark breath", 6);

    set_skill("conjuration", zspell*zlevel);
    add_spell("icefire breath", "$A"); //AOE
    set_spell_level("icefire breath", 6);

    set_skill("owedquar's will", zspell*zlevel);
    add_spell("violent blaze", "$A"); //AOE
    set_spell_level("violent blaze", 6);
    add_spell("storm of ages", "$A"); //AOE
    ...
    add_spell("destruction", "$A");
    set_spell_level("destruction", 6);
    add_spell("annihilation", "$A");
    set_spell_level("annihilation", 6);


    ob = new("/std/spells/shadows/ele_pres_shad");
    ob->set_power(6);
    ob->set_damage(({ damage1, damage2 }));
    ob->set_shadow_name("Elder Brain Presence");
    ob->start_shadow(this_object(), 2000000000, "");

    ob = new("/std/spells/shadows/ele_pres_shad");
    ob->set_power(6);
    ob->set_damage(({ "infernal", "aether" }));
    ob->set_shadow_name("Elder Brain Presence2");
    ob->start_shadow(this_object(), 2000000000, "");

    ob = new("/std/spells/shadows/ele_pres_shad");
    ob->set_power(6);
    ob->set_damage(({ "holy", "unholy" }));
    ob->set_shadow_name("Elder Brain Presence3");
    ob->start_shadow(this_object(), 2000000000, "");

    ob = new("/d/areas/beholders/spells/ed_shield.c");
    ob->set_damage_allowed(25000);
    ob->set_shadow_name("Elder Brain shield");
    ob->start_shadow(this_object(), 2000000000, "");
...
message("info","%^COLOR200%^Very Rare loot drops on the corpse you should probably LOOK at corpse and get it%^RESET%^", environment(this_object()), this_object());
    ob = new("/d/1tlny/loot/multi_skill_ring");
...
int no()
{
   ...
   if(this_player()->query_ghost()) return 0;
    message("blocking",
      "The Elder Brain moves in front of you, blocking your path.\n",
      this_player());
    message("blocking",
      "The Elder Brain blocks "+capitalize(this_player()->query_name())+"'s path.\n",
