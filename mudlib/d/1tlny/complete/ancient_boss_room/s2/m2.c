...
switch(random(7)){
   ...
   set_level(zlevel);
   set_name("Skaven");
   set("id", ({ "skaven", "Skaven", "rat" }));
   set_short("["+zlevel+"]Gigantic Skaven");
   set("long",
	"You see a mountain of a Skaven before you what is that!!");
...
if (loot == 16) {  
//special GEAR DROP can cast haste
    ob = new("/d/1tlny/loot/orb_haste");
    ob->move(this_object()); 
} 
else
if (loot == 17) {
