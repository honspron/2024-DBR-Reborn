...
switch(random(7)){
   ...
   set_level(zlevel);
   set_name("goblin");
   set("id", ({ "orc", "Orc", "gigantic orc" }));
   set_short("["+zlevel+"]Gigantic orc");
   set("long",
	"You see a mountain of a Orc before you what is that!!");
...
if (loot == 16) {  
//special GEAR DROP can cast enlarge
    ob = new("/d/1tlny/loot/orb_enlarge");
    ob->move(this_object()); 
} 
else
if (loot == 17) {
