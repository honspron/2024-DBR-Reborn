...
void create() {
    ...
    set_long("This is a perfectly spherical glowing orb approximately nine inches in diameter. it appears like nothing but maybe if you did the commad%^YELLOW%^'orb frgrasp'%^RESET%^somthing might happen\n");
    set_weight(10);
    set_value(1);
    return;
}
...
int fun_orb(string mesg) {
   ...
   if(time() - last_used >= HOUR || !last_used) {
      last_used = time();
      ob = new("/std/spells/freezing_grasp");
      ...
      write("You cast *6 freezing grasp");
