...
switch(random(6)){
   ...
   set_short("A ring of supreme %^COLOR196%^"+zstat+"%^RESET%^ and increased "+skill1+" skill");
...
int extra_worn(){
   write("You feel the energy of the ring surge through your body. You feel a sensation of increaased "+zstat+" and skill in "+skill1+" as you wear this ring");
  this_player()->add_stat_bonus(zstat, 50);
    TP->add_skill_bonus(skill1, 40);
  return 1;
}

int extra_remove(){
   write("You feel the energy of the ring releases from your body. You feel drained as your "+zstat+" and skill in "+skill1+" is decreased");
  this_player()->add_stat_bonus(zstat, -50);
    TP->add_skill_bonus(skill1, -40);
  return 1;
}
