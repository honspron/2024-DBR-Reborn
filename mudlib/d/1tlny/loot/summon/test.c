...
tp = this_player();
op = tp;
qlevel=(int) op->query_level();
//ob = find_player;

//title = (string) ob->query_level();



//zlevel=(qlevel+random(3));
zlevel=((find_player)this_player()->query_level());



  //set_level(zlevel);
  set_level((int)this_player()->query_level());
  set_name("dracolich");
  ...
  set_overall_ac(20);
  set_overall_ac(4);
  set_short("["+zlevel+"]ominous Dracolich");
...
void set_up(int pow) {
  ...
  set_property("melee damage", ([ "unholy" : zlevel+random(10) ]) );
