...
void create() {
  ...
  set_long("This is the storage room for the Adventurer's General Store. Lots of "
  ...
  add_exit(ROOMS+"/shop/general_store", "west");
  set_door("iron door", ROOMS+"/shop/general_store", "west","fighter store key");
...
void reset() {
  ...
  bob = present("shopkeeper", find_object_or_load(ROOMS+"shop/general_store"));
  if(bob) {
