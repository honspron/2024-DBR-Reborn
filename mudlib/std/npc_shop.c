...
"---------------------------------------------------------------------\n", cust);
  ...
  if(cmd == "help") {
    say_this("If you wanna buy something, say shopkeeper, offer <amt> gold, <amt> mithril, etc. for <item>", cust);
...
void sell_item(string str, object who) {
  ...
  if(str == "all") {
	...
	continue;
      ...
      amt = mtmp[0];
      if(!amt) continue;
      amt += amt * ((int)who->query_skill("trade lore") - random(36)) / 200;
...
void appraise_obj(string what, object who) {
  ...
  tmp = money_val(ob);
  gold_equiv = tmp[0];
  gold_equiv += ((int)who->query_skill("trade lore") - random(36)) * gold_equiv / 200;
  if(gold_equiv > 0)
...
void offer_me(string arg, object who) {
  ...
  item_val -= (to_float(((int)who->query_skill("trade lore") - random(36)) / 2) * item_val) / 130.0;
