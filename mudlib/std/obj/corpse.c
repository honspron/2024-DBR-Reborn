...
varargs void preserve_me(int extra) {
  remove_call_out("decay");
  remove_call_out("decay");
  set("preserve time", time());
  if(extra) call_out("decay", extra);
  return;
}
...
int query_elapsed() {
  if(!query("create time")) return 0;
  if(!query("preserve time")) return (time() - query("create time"));
  return (query("preserve time") - query("create time"));
}
