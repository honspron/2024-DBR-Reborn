...
void set_up(int pow) {
  ...
  set_level(10 + pow/3);
  set_short((string)query_property("material_name")+" Golem");
  set_hp( 600 + pow*55 + (int)prop("hardness")/4);
  set_mp( 300 + pow*55);
  set_skill("melee", 70 + 4*pow + (int)prop("hit bonus"));
  set_skill("dodge", 50 + 3*pow);
  set_skill("parry", 60 + 4*pow);
  set_stats("strength", 80 + 6 * pow);
  tmp = ([]);
  ...
  set_property("melee damage", tmp + ([ "crushing" : 20 + (4 * pow)
...
void improve_me(string package, int pow) {
  ...
  case "melee damage":
    set_melee_damage( (mapping)query_property("melee damage") +
     ([ "cutting" : 25+(4*pow)+(int)prop("damage bonus")*(pow/3+1) ]) );
    set_stats("strength", 85 + 4*pow);
    break;
  case "damage spells":
    set_combat_chance(50);
    set_skill("conjuration", 65 + 4*pow);
    ...
    break;
  case "healing spells":
    set_casting_chance(45);
    set_skill("prayer", 65+3*pow);
    ...
    break;
  case "hit points":
    set_hp(400 + 65*pow + (1+pow/3)*(int)prop("hardness") / 4);
    break;
  case "magic points":
    set_mp(400 + 60*pow);
    break;
  case "armour":
    set_overall_ac(6*pow + (int)prop("armour bonus")*2);
    break;
  case "protection spells":
    set_casting_chance(40);
    set_skill("elementalism", 60+3*pow);
    add_spell("elemental barrier", "$(ME)", 1, 245*pow);
    set_spell_level("elemental barrier", pow);
    if(!query_skill("prayer")) set_skill("prayer", 60+3*pow);
