...
void set_up(int pow) {
  ...
  set_skill("melee", 35 + 3*pow + (int)prop("hit bonus"));
  set_skill("dodge", 10 + 2*pow);
  set_skill("parry", 15 +3*pow);
  ...
  set_property("melee damage", tmp + ([ "crushing" : 4 + (2 * pow)
...
void improve_me(string package, int pow) {
  ...
  case "melee damage":
    set_melee_damage( (mapping)query_property("melee damage") +
     ([ "cutting" : 5+(3*pow)+(int)prop("damage bonus") ]) );
    set_stats("strength", 40+4*pow);
    break;
  case "damage spells":
    set_combat_chance(45);
    set_skill("conjuration", 30+3*pow);
  ...
  case "healing spells":
    ...
    set_skill("prayer", 25+3*pow);
    set_spell_level("cure light wounds", 2+pow/2);
    add_spell("cure light wounds", "$(ME)");
    break;
  case "hit points":
    set_hp(175*pow + (int)prop("hardness") / 8);
    break;
  case "magic points":
    set_mp(200+35*pow);
    break;
  case "armour":
    set_overall_ac(3*pow + (int)prop("armour bonus"));
    break;
