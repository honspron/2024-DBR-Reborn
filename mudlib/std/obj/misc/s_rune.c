...
void do_gate(object who) {
  ...
  ob = new("/std/obj/pet/demon_lord");
  ob->set_owner("asskicker");
  ob->set_up(query_level());
  ob->set_dur(query_level() * 60);
  message("info", "%^GREEN%^%^BOLD%^A Demon Lord appears!",
    environment(who));
  ob->move(environment(who));
  if(random(100) >= ((int)who->query_skill("control demon")-20*query_level())) {
    ob->kill_ob(who, 0);
