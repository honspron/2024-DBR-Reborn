...
//TLNY2024 added  && !victim->query_ghost())
...
//TLNY2024 possible cool code to add for IK resets
/*
...
//special combat tracker added to remove possible infinite recurssion
	...
	if(num_rounds <= 0 && ldr->query_ghost() == 0 && skill_contest((int)ldr->query_skill("body block"),
	    (2*(int)me->query_stats("strength")/3), 1) != 2 && (!ldr->query_property("no bodyblock"))) {
