...
void info() {
message("help",
"This skill is used to inscribe a inferner rune on a weapon."
"This rune adds infernal damage to the weapon permanently.  The amount of damage "
...
"by 'use'-ing the skill again.\n"
  "Syntax: use inscribe inferner at <weapon>",
        this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("inferner", tmp) >= 0) {
    message("info", "That weapon already has a inferner rune on it.", from);
    remove();
    return;
  }
...
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
    message("info", "A %^B_RED%^Inferner%^RESET%^ rune appears on the weapon.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "inferner" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
    if(pointerp(tmp)) tmp += ({ "A %^B_RED%^Inferner%^RESET%^ rune is inscribed on the weapon." });
    else if(stringp(tmp)) tmp = ({ tmp, "A %^B_RED%^Inferner%^RESET%^ rune is inscribed on the weapon." });
  else tmp = ({ "A %^B_RED%^Inferner%^RESET%^ rune is inscribed on the weapon." });
