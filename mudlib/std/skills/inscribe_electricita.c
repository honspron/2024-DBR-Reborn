...
void info() {
message("help",
"This skill is used to inscribe a electricita rune (physical protection rune) on an armour.  "
"This adds physical electricity protection to the armour.  The potency of the rune "
...
"Syntax: use inscribe electricita at <armour>",
	this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("electricita", tmp) >= 0) {
    message("info", "That armour already has a electricita rune on it.",
      from);
    remove();
    return;
  }
...
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
  message("info", "A %^CYAN%^%^BOLD%^electricita%^RESET%^ rune appears on the armour.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "electricita" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
  if(pointerp(tmp)) tmp += ({ "A %^CYAN%^%^BOLD%^Electricita%^RESET%^ rune is inscribed on the armour." });
  else if(stringp(tmp)) tmp = ({ tmp, "A %^CYAN%^%^BOLD%^Electricita%^RESET%^ rune is inscribed on the armour." });
  else tmp = ({ "A %^CYAN%^%^BOLD%^Electricita%^RESET%^ rune is inscribed on the armour." });
