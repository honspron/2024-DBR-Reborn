...
void info() {
message("help",
"This skill is used to inscribe a quickness rune on a weapon.  "
...
"Syntax: use inscribe quickness at <weapon>",
	this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("quickness", tmp) >= 0) {
    message("info", "That weapon already has a quickness rune on it.",
      from);
    remove();
    return;
  }
...
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
  message("info", "A %^GREEN%^%^BOLD%^Quickness%^RESET%^ rune appears on the weapon.", from);
  ...
  if(pointerp(tmp)) tmp += ({ "A %^B_GREEN%^%^BOLD%^Quickness%^RESET%^ rune is inscribed on the weapon." });
  else if(stringp(tmp)) tmp = ({ tmp, "A %^B_GREEN%^%^BOLD%^Quickness%^RESET%^ rune is inscribed on the weapon." });
  else tmp = ({ "A %^B_GREEN%^%^BOLD%^Quickness%^RESET%^ rune is inscribed on the weapon." });
