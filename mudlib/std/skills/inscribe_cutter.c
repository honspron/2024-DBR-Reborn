...
void info() {
message("help",
"This skill is used to inscribe a cutter rune on a weapon."
...
"by 'use'-ing the skill again.\n"
  "Syntax: use inscribe cutter at <weapon>",
        this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("cutter", tmp) >= 0) {
    message("info", "That weapon already has a cutter rune on it.", from);
    remove();
    return;
  }
...
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
    message("info", "A %^BOLD%^Cutter%^RESET%^ rune appears on the weapon.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "cutter" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
    if(pointerp(tmp)) tmp += ({ "A %^BOLD%^Cutter%^RESET%^ rune is inscribed on the weapon." });
    else if(stringp(tmp)) tmp = ({ tmp, "A %^BOLD%^Cutter%^RESET%^ rune is inscribed on the weapon." });
  else tmp = ({ "A %^BOLD%^Cutter%^RESET%^ rune is inscribed on the weapon." });
