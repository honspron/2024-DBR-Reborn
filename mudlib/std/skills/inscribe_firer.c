...
void info() {
message("help",
"This skill is used to inscribe a firer rune on a weapon."
"This rune adds fire damage to the weapon permanently.  The amount of damage "
...
"by 'use'-ing the skill again.\n"
  "Syntax: use inscribe firer at <weapon>",
        this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("firer", tmp) >= 0) {
    message("info", "That weapon already has a firer rune on it.", from);
    remove();
    return;
  }
...
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
    message("info", "A %^BOLD%^%^RED%^Firer%^RESET%^ rune appears on the weapon.", from);
