...
void info() {
message("help",
"This skill is used to inscribe a inferna rune (physical protection rune) on an armour.  "
"This adds physical infernal protection to the armour.  The potency of the rune "
...
"Syntax: use inscribe inferna at <armour>",
	this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("inferna", tmp) >= 0) {
    message("info", "That armour already has a inferna rune on it.",
      from);
    remove();
    return;
  }
...
void finish_work(object from, object at) {
  ...
  wc["infernal"] += skill/2+skill/2+skill/3+skill/4+skill/5+skill/6+(this_player()->query_level()/2);
  wc_keys = keys(wc);
  i = sizeof(wc_keys);
  while(i--)
    at->set_ac(wc[wc_keys[i]], wc_keys[i]);
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
  message("info", "A %^B_RED%^%^BOLD%^inferna%^RESET%^ rune appears on the armour.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "inferna" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
  if(pointerp(tmp)) tmp += ({ "A %^B_RED%^%^BOLD%^Inferna%^RESET%^ rune is inscribed on the armour." });
  else if(stringp(tmp)) tmp = ({ tmp, "A %^B_RED%^%^BOLD%^Inferna%^RESET%^ rune is inscribed on the armour." });
  else tmp = ({ "A %^B_RED%^%^BOLD%^Inferna%^RESET%^ rune is inscribed on the armour." });
