...
void info() {
message("help",
"This skill is used to inscribe a unholyer rune on a weapon."
"This rune adds unholy damage to the weapon permanently.  The amount of damage "
...
"by 'use'-ing the skill again.\n"
  "Syntax: use inscribe unholyer at <weapon>",
        this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("unholyer", tmp) >= 0) {
    message("info", "That weapon already has a unholyer rune on it.", from);
    remove();
    return;
  }
...
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
    message("info", "A %^BLACK%^%^B_BLUE%^Unholyer%^RESET%^ rune appears on the weapon.", from);
