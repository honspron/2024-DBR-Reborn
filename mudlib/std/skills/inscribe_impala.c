...
void info() {
message("help",
"This skill is used to inscribe a impala rune (physical protection rune) on an armour.  "
"This adds physical impaling protection to the armour.  The potency of the rune "
...
"Syntax: use inscribe impala at <armour>",
	this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("impala", tmp) >= 0) {
    message("info", "That armour already has a impala rune on it.",
      from);
    remove();
    return;
  }
...
void finish_work(object from, object at) {
  ...
  wc["impaling"] += skill/2+skill/2+skill/3+skill/4+skill/5+skill/6+(this_player()->query_level()/2);
  wc_keys = keys(wc);
  i = sizeof(wc_keys);
  while(i--)
    at->set_ac(wc[wc_keys[i]], wc_keys[i]);
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
  message("info", "A %^B_GREEN%^%^BOLD%^Impala%^RESET%^ rune appears on the armour.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "impala" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
  if(pointerp(tmp)) tmp += ({ "A %^B_GREEN%^%^BOLD%^Impala%^RESET%^ rune is inscribed on the armour." });
  else if(stringp(tmp)) tmp = ({ tmp, "A %^B_GREEN%^%^BOLD%^Impala%^RESET%^ rune is inscribed on the armour." });
  else tmp = ({ "A %^B_GREEN%^%^BOLD%^Impala%^RESET%^ rune is inscribed on the armour." });
