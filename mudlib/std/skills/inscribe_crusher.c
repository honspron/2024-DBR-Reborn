...
void info() {
message("help",
"This skill is used to inscribe a crusher rune on a weapon."
"This rune adds crushing damage to the weapon permanently.  The amount of damage "
...
"by 'use'-ing the skill again.\n"
  "Syntax: use inscribe crusher at <weapon>",
        this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("crusher", tmp) >= 0) {
    message("info", "That weapon already has a crusher rune on it.", from);
    remove();
    return;
  }
...
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
    message("info", "A %^ORANGE%^Crusher%^RESET%^ rune appears on the weapon.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "crusher" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
    if(pointerp(tmp)) tmp += ({ "A %^ORANGE%^Crusher%^RESET%^ rune is inscribed on the weapon." });
    else if(stringp(tmp)) tmp = ({ tmp, "A %^ORANGE%^Crusher%^RESET%^ rune is inscribed on the weapon." });
  else tmp = ({ "A %^ORANGE%^Crusher%^RESET%^ rune is inscribed on the weapon." });
