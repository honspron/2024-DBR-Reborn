...
//TLNY 2020 This is kind of Magic and melee AC added at a lower value
  ...
  wc["impact"] += skill/2+skill/2+skill/3+skill/4+skill/6+skill/7+(this_player()->query_level()/2);
 if(!wc["strike"]) wc["strike"] = 0;
  wc["strike"] += skill/2+skill/2+skill/3+skill/4+skill/6+skill/7+(this_player()->query_level()/2);
  wc_keys = keys(wc);
  i = sizeof(wc_keys);
  while(i--)
