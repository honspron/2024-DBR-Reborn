...
void info() {
message("help",
"This skill is used to inscribe a fira rune (physical protection rune) on an armour.  "
"This adds physical fire protection to the armour.  The potency of the rune "
...
"Syntax: use inscribe fira at <armour>",
	this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("fira", tmp) >= 0) {
    message("info", "That armour already has a fira rune on it.",
      from);
    remove();
    return;
  }
...
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
  message("info", "A %^RED%^%^BOLD%^fira%^RESET%^ rune appears on the armour.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "fira" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
  if(pointerp(tmp)) tmp += ({ "A %^RED%^%^BOLD%^Fira%^RESET%^ rune is inscribed on the armour." });
  else if(stringp(tmp)) tmp = ({ tmp, "A %^RED%^%^BOLD%^Fira%^RESET%^ rune is inscribed on the armour." });
  else tmp = ({ "A %^RED%^%^BOLD%^Fira%^RESET%^ rune is inscribed on the armour." });
