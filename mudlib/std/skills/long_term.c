...
*/
  ...
  if(time_needed>0) {
    who->set("long term", time_needed);
  } else {
    set("long term", 1);
  }
  who->set_property("long term skill name", skill_name);
  if(!query_in_progress(what)) {
    what->set_property("progress "+skill_name+"_"+worker_name,0);
    who->set_property("long term progress", 0);
    ...
    who_working->set_property("long term progress", (int)what->query_property("progress "+skill_name+"_"+worker_name));
...
*/
  ...
  who_working->set_property("long term progress", (int)working_on->query_property("progress "+skill_name+"_"+worker_name));
  if(time_to_go <= 0) {
    message("info", "%^BOLD%^%^CYAN%^You finish your work.", who_working);
    working_on->remove_property("progress "+skill_name+"_"+worker_name);
    who_working->remove_property("long term progress");
    who_working->remove_property("long term skill name");
    who_working->set("long term", 0);
    if(objectp(my_shadow)) my_shadow->external_destruct(my_shadow);
    if(objectp(your_shadow)) your_shadow->external_destruct(your_shadow);
...
// Re-added long term EXP TLNY 2020
 who_working->add_exp(((who_working->query_level())*(3*skill_level))+random(2*skill_level));
