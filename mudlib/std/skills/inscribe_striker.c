...
void info() {
message("help",
"This skill is used to inscribe a striker rune on a weapon."
"This rune adds strike damage to the weapon permanently.  The amount of damage "
...
"by 'use'-ing the skill again.\n"
  "Syntax: use inscribe striker at <weapon>",
        this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("striker", tmp) >= 0) {
    message("info", "That weapon already has a striker rune on it.", from);
    remove();
    return;
  }
...
void finish_work(object from, object at) {
  ...
  wc["strike"] += skill/2+skill/2+skill/3+skill/4+skill/5+skill/6+(this_player()->query_level()/2);
  wc_keys = keys(wc);
  i = sizeof(wc_keys);
  while(i--)
    at->set_wc(wc[wc_keys[i]], wc_keys[i]);
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
    message("info", "A %^COLOR217%^Striker%^RESET%^ rune appears on the weapon.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "striker" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
    if(pointerp(tmp)) tmp += ({ "A %^COLOR217%^Striker%^RESET%^ rune is inscribed on the weapon." });
    else if(stringp(tmp)) tmp = ({ tmp, "A %^COLOR217%^Striker%^RESET%^ rune is inscribed on the weapon." });
  else tmp = ({ "A %^COLOR217%^Striker%^RESET%^ rune is inscribed on the weapon." });
