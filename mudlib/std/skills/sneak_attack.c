...
"for fighters with the 'reverse stroke' skill...." 
"Repeated attack attempts at the "
...
void skill_func(object from, object at, string arg) {
  ...
  tmp_pi = (int *)at->query("sneak attack bonus:"+(string)from->query_name());
 
  if(!tmp_pi) bonus = 0;
  else if(sizeof(tmp_pi) != 2) bonus = 0;
  else if((time() - tmp_pi[1]) > HOUR) bonus = 0;
  else bonus = tmp_pi[0];
  at->set("sneak attack bonus:"+(string)from->query_name(),
...
skillfully with your "+
  ...
  if(roll == 2) {
    ...
    message("other_action", "You spy "+(string)from->query_cap_name() +
            " just as "+nominative(from)+" is about to launch a sneak attack!",
            at);
    message("other_action", (string)from->query_cap_name() +
            " attempts to attack "+(string)at->query_cap_name() +
            " from behind, but is thwarted.", environment(from),
            ({ from, at }));
    at->kill_ob(from, 0);
    remove();
    return;
  }
  message("info", "You attack "+
          (string)at->query_cap_name() + "from behind.", from);
  message("info", "You feel a horrendous pain as "+
          (string)from->query_cap_name() + " does "
          " a cowardly sneak attack", at);
  message("info", "Without warning, "+(string)from->query_cap_name() +
          " jumps from the shadows, using "+possessive(from) +
          " weapon in a sneak attack on "+(string)at->query_cap_name() +
          "!", environment(from), ({ from, at }) );
  at->kill_ob(from, 0);
  
   do_stab(from, at, (mapping)from->get_damage(weap), props["skill
level"]);
...
void do_stab(object from, object at, mapping wc, int skill) {
  ...
  i = sizeof(wc_keys);
  crits = ({});
  pierce = skill / 6 + random(20) + random(20);
  while(i--) {
    ...
    dmg = 0;
    for(j=1;j <= 9;j++) dmg += 1+random(6+skill/7);
    dmg -= prot*dmg/100;
    ...
    if(dmg >= 0 && wc_keys[i] == "impaling") {
      mesgs = (string *)DAMAGE_D->attack_message(sprintf(
                "%d %s %s:%s", dmg,
((rev_flag)?"reverse-strike":"sneak attack"),
