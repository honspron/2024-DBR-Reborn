...
void info() {
message("help",
"This skill is used to inscribe a colda rune (physical protection rune) on an armour.  "
"This adds physical cold protection to the armour.  The potency of the rune "
...
"Syntax: use inscribe colda at <armour>",
	this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("colda", tmp) >= 0) {
    message("info", "That armour already has a colda rune on it.",
      from);
    remove();
    return;
  }
...
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
  message("info", "A %^BLUE%^%^BOLD%^colda%^RESET%^ rune appears on the armour.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "colda" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
  if(pointerp(tmp)) tmp += ({ "A %^BLUE%^%^BOLD%^Colda%^RESET%^ rune is inscribed on the armour." });
  else if(stringp(tmp)) tmp = ({ tmp, "A %^BLUE%^%^BOLD%^Colda%^RESET%^ rune is inscribed on the armour." });
  else tmp = ({ "A %^BLUE%^%^BOLD%^Colda%^RESET%^ rune is inscribed on the armour." });
