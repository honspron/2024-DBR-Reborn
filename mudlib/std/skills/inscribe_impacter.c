...
void info() {
message("help",
"This skill is used to inscribe a impacter rune on a weapon."
"This rune adds impact damage to the weapon permanently.  The amount of damage "
...
"by 'use'-ing the skill again.\n"
  "Syntax: use inscribe impacter at <weapon>",
        this_player());
}
...
void skill_func(object from, object at, string arg) {
  ...
  if(member_array("impacter", tmp) >= 0) {
    message("info", "That weapon already has a impacter rune on it.", from);
    remove();
    return;
  }
...
void finish_work(object from, object at) {
  ...
  wc["impact"] += skill/2+skill/2+skill/3+skill/4+skill/5+skill/6+(this_player()->query_level()/2);
  wc_keys = keys(wc);
  i = sizeof(wc_keys);
  while(i--)
    at->set_wc(wc[wc_keys[i]], wc_keys[i]);
from->add_exp2(15 * props["skill level"]+(this_player()->query_level()*100));
    message("info", "A %^CYAN%^Impacter%^RESET%^ rune appears on the weapon.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "impacter" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
    if(pointerp(tmp)) tmp += ({ "A %^CYAN%^Impacter%^RESET%^ rune is inscribed on the weapon." });
    else if(stringp(tmp)) tmp = ({ tmp, "A %^CYAN%^Impacter%^RESET%^ rune is inscribed on the weapon." });
  else tmp = ({ "A %^CYAN%^Impacter%^RESET%^ rune is inscribed on the weapon." });
