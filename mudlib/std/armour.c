...
//TLNY2020 ADD
int __Wear(string str);
int __Unwear(string str);
...
void init() {
    ...
    add_action("__Wear", "wear");
    add_action("__Unwear", "remove");
...
//TLNY2020 ADD to make drop code and give code work properly with spells ups
//add 
void unequip() { 
  if(armour_static["worn by"]) {
    if(functionp(armour_static["unwear"]))
      evaluate(armour_static["unwear"]);
    else if(pointerp(armour_static["unwear"]))
      map_array(filter_array(armour_static["unwear"],
			     (: functionp :)),
		(: evaluate :));
  }
  unwear();
...
void unwear() {
  //add
  if(!armour_static["worn by"]) return;
  armour_static["worn by"]->remove_armour_from_limb(this_object(), armour_static["actual limbs"]);
  //end
    ...
    message("other_action", (string)armour_static["worn by"]->query_cap_name()+
      ...
      +query_short()+".", environment(armour_static["worn by"]),
      ({ armour_static["worn by"] })); 
    set_not_equipped();
}
//TLNY2020 redone 
