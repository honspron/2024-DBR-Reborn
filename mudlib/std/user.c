...
// Modified by Khojem 093096 stop ghosts from seeing room names
...
string *query_quests();
private static int register_channels();
private static int setup_inform();
...
//    if((light=effective_light(this_object())) > 6 || light < 1) {
  if((light=effective_light(this_object())) > 15 || light < 1) {
	if(light > 15) borg += "It is too bright to see.";
...
void set_property(string idx, mixed val) {
    ...
    if(member_array(idx, LOG_PROPS) >= 0 && !wizardp(this_object())
      ...
      "/d/standard/setter") {
	if(this_player())
          who = (string)this_player()->query_true_name();
        else
          who = "unknown";
        log_file("illegal", (string)this_object()->query_true_name()+
        ":"+idx+
        ":"+val+
...
varargs void net_dead(int flag) {
    ...
    if(!hiddenp(this_object()))
	  ...
	  INFORM_D->do_inform("net_dead_players","Info: " +
	  capitalize((string)this_object()->query_name()) +
          " suckered out and went net dead %^BOLD%^%^YELLOW%^IN COMBAT%^RESET%^!!!", who_exc);
    channels = ({});
...
nomask void die() {
    ...
    if(ob=this_object()->query_casting()) {
      ob->remove();
      this_object()->set_casting(0);
    }
...
void set_position(string pos) {
    ...
    if(this_player())
      ...
      who = "unknown";
    log_file("security", "set_position"+
      ":"+this_object()->query_true_name()+
      ":"+getuid(previous_object())+
      ":"+ ctime(time())+
      ":"+this_object()->query_ip()+
      ":"+who);
    if(geteuid(previous_object()) != UID_ADVANCE && geteuid(previous_object()) != UID_ROOT) {
      log_file("security", ":FAIL"+"\n");
      return;
    }
...
nomask void reset_quests() {
    ...
    if(this_player())
      ...
      who = "unknown";
    log_file("illegal", this_object()->query_true_name()+
    ":reset_quests"+
    ":"+file_name(previous_object())+
    ":"+getuid(previous_object())+
    ":"+ctime(time())+
...
static private int setup_inform() {
	...
	inf_heard += ({ "external_edits","user_shadows","watch_register" });
...
// TLNY2022 added 2 rounds of combat for fighters
int query_num_rounds() {
 if (query_class() == "fighter") return (2);
  else return 1;
}
