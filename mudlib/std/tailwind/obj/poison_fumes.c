...
void create() {
	...
	"perfume"}) );
    set_long("A small jar containing poison perfume. You can apply it, but
it won't do anything... it needs magic activation.");
   set_weight(3);
   set_value(0);
   call_out("setup_potion", 2);
}
...
&& (str != "poison perfume") && (str != "perfume"))) {
      notify_fail("smash what?\n");
      return 1;
   }

   if((time() - (int)this_player()->query_last_use("smash potion")) < 8){
      write("It takes time to smash another potion.");
      return 1;
   }
   this_player()->set_last_use("smash potion");
   ...
   if(query_potion_power() < 1 || query_potion_duration() < 1){
    ...
    ob->start_shadow(this_player(), query_potion_duration(), 
        "%^BOLD%^" + "%^GREEN%^" + "Speed Wears Off");

    write("%^BOLD%^" + "%^GREEN%^" + 
        "You smash "+query_short()+" and poison starts to fill the room");
    ...
    message("info", "%^BOLD%^" + "%^RED%^" + 
        ...
        call_out("damage_all", 5, base_name(environment(cas)),
            query_potion_power() * 4);
   }
   set_short("");
   move(environment(cas));
...
void damage_all(string room, int times) {
  ...
  while(i--) {
    ...
    inv[i]->add_poisoning(query_potion_power() * 8);
    if(cas)
      inv[i]->kill_ob(cas, 1);
  }
  if(times <= 0) {
