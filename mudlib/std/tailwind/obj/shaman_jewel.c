...
void set_up(){
	...
	break;
    case 31..40:
	set_short("A brightly-glowing "+ color +" "
	  +this_object()->query_flavor());
	break;
    case 41..60:
	set_short("A shining "+ color +" "+this_object()->query_flavor());
	break;
    case 61..70:
	set_short("A brightly-shining "+ color +" "
	  +this_object()->query_flavor());
	break;
    case 71..80:
	set_short("A blindingly-bright "+ color +" "
	  +this_object()->query_flavor());
	break;
    case 81..90:
    ...
    set_id( ({ (string)this_object()->query_flavor(), "shaman "+(string)this_object()->query_flavor(), (string)this_object()->query_color()+" jewel", (string)this_object()->query_type()+" jewel" }) );
    set_long("This "+(string)this_object()->query_flavor()+
      " contains the power of "+(string)this_object()->query_type()+
      " within it. It is made of "
      +this_object()->query_property("material_name")+".");
    this_object()->set_type(gemtype);
    this_object()->set_color(color);
    this_object()->set_property("new desc", this_object()->query_short());
    this_object()->set_property("new long", this_object()->query_long());
    this_object()->set_property("new ids", this_object()->query_id());

    if((gemtype == "time") && (!query("timecharging"))){
	call_out("time_charge", 1800);
	set("timecharging", 1);
    }
    recursion = 0;
    return;
...
int time_charge() {
    this_object()->set_property("charge", this_object()->query_property("charge")+10);
...
string set_flavor(string flavor) {return this_object()->set_property("flavor", flavor );}
