...
int burn_blanket(string str) {
    ...
    if((ob->query_name() != "blanket")) {
	write("You can only burn blankets in the campfire.");
	return 1;
    }
    write("You ignite the blanket. You better 'smother' someone with it quickly before it is consumed!");
    ob->remove();
    ob = new("/std/tailwind/obj/burning_blanket");
    ob->move(this_player());
    return 1;
}
