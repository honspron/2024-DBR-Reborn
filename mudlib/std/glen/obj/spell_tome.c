...
void create() {
    ...
    set_short("A tome of knowledge");
    set_long("This tome is used by mages to record spell knowledge.");
    set_id( ({ "scroll","parchment scroll","spell scroll","tome","spell tome" }) );
    set_name("scroll");
    set_weight(4);
    set_value(200);
    return;
...
string query_long(string null) {
    ...
    res += "\n\n";
    res += "This tome has the spell: "+sprintf("%s *%-d", spell,
      spell_pow)+
    " inscribed upon it.  Cast it with 'read <spell> from <tome> " +
    "at <args>'." +
...
int cast_spell(string str) {
	...
	if(sscanf(str, "%s from %s", name, me) != 2)
             return notify_fail("Read what spell from what tome?\n");
    }
    if(present(me, this_player()) != this_object())
	return notify_fail("You do not have "+article(me)+".\n");
    if(!spell || spell != name) {
         write("That spell is not scribed on this tome.");
	return 1;
    }
...
void remove_skill(object who) {
    ...
    uses--;
    if(uses > 0)return;
    message("info","A tome crumbles into nothingness.", who);
    remove();
    return;
}
