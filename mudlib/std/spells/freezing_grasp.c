...
void create() {
    ::create();
    set_property("name","freezing grasp");
    ...
    set_property("caster message","You cast freezing grasp on $T.");
    set_property("target message","$C casts freezing grasp on you.");
    set_property("observer message","$C casts freezing grasp on $T.");
...
void info() {
message("help",
"This spell gives the recipient hands that radiate cold,freezing"
...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  if(!flag) {
    ob = new("/std/spells/shadows/fz_grasp_shadow");
    ob->set_power(power);
