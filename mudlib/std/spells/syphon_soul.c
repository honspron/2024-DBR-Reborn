...
void create() {
    ::create();
    set_property("name","syphon soul");
    set_property("skill","black magic");
    set_property("casting time",10);
    set_property("base mp cost",155);
    set_property("dev cost", 155);
    set_property("fast dev cost", 465);
    set_property("spell level", 30);
    set_property("moon","luna");
    set_property("caster message","You cast syphon soul at $T.");
    set_property("target message","$C begins to syphon your soul!");
    set_property("observer message","$C casts soul syphon at $T.");
    set_property("spell type",({ }));
    set_property("target type", "living");
    set_property("must be present", 1);
    set_property("combat spell", 1);
    set_property("prereq", "lifesteal");
...
"at gives it to you. "
"For the length of the duration, the you syphon some of the targets hp/mp.",
this_player());
}
...
void spell_func(object caster, object at, int power, string args, int flag) {
  sp_pow = power;
  if(flag) {
    message("info", "%^GREEN%^You accidentally syphon away a good deal of your hit points and mana points!",
      caster);
...
//  int i;
  ...
  if(times <= 0) {
    at->set("syphon #", (int)at->query("syphon #") - 1);   
    message("info", "%^CYAN%^%^BOLD%^A syphon soul spell wears off.",
        at); 
    remove();
    return;
  }
