...
void create() {
    ::create();
    set_property("name", "rotted zombie infusion");
...
void info() {
    message("help", "This spell permits a Nethermancer to infuse the soul of a Rotted Zombie into his or her soul orb. Upon completion of the infusion, the infused soul can be joined with other souls using soul union magic or the orb as a whole can be finalized to bring the soul to life as a pet or sacrifice it for temporary magical power. To cast this spell, a charged lesser nexus is required to be present in the room along with a soul orb to cast the spell at (available to buy at magic shops). This is a long-term spell (see help long-term), but its overall time can be reduced by the latent skill 'minor nether arts'. If you are interrupted while casting, you may resume the casting at a later time provided a nexus and the same soul orb is available.\n\nSyntax: 'cast *6 rotted zombie infusion at orb'", this_player());
}
...
void spell_func(object caster, object at, int power, string args, int flag) {
    ...
    int finaltime = (2400/power) - bonus;
...
void finish_work(object caster, object at, int power) {
    ...
    message("info", "You finish infusing a %^BOLD%^%^RED%^rotted zombie%^RESET%^ soul into the soul orb!", caster);
    message("info", ""+(string)caster->query_cap_name()+" successfully infuses a Rotted Zombie soul into a soul orb.", environment(caster), ({ caster }));
