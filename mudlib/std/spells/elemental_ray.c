...
void create() {
    ::create();
    set_property("name","elemental ray");
    ...
    set_property("caster message","You cast elemental ray at $T.");
    set_property("target message","");
    set_property("observer message","$C casts elemental ray at $T.");
...
void info() {
message("help",
"This spell does damage to a target. The type of damage depends on the type of jewel used in casting the spell. It does more damage than gaia blast, but also drains much more power out of the jewel.\n"
"Syntax: cast *<pow> elemental ray at <target> with <jewel>\n"
"Ex: cast *2 elemental ray at guard with jewel\n",
this_player());
}
...
{
  jewel = present(lower_case(args), caster);
  jewel->subtract_charge(random(9)+3*power);
  jewel->set_up();


staff_set(jewel->query_material());

    set_property("damage types", ([ damtype : 120 ]));
