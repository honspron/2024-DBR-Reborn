...
void create() {
    ...
    set_property("fast dev cost",123);
    ...
    set_property("prereq", "lesser demon sword");
    return;
}
...
lvl = (caster->query_level()/2); 
  ...
  ob->set_extra_wc(([ "impact" : 8*power+lvl, "disruption" : 5*power+lvl ]));
