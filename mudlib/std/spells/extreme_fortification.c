...
void create() {
    ::create();
    set_property("name", "extreme fortification");
    set_property("skill","enchantment");
    set_property("duration", "permanent");
    set_property("casting time",4);
    set_property("base mp cost",140);
    set_property("dev cost", 240);
    set_property("fast dev cost", 720);
    set_property("spell level", 47);
    ...
    set_property("prereq", "master fortification");
    return;
}

void info() {
message("help",
"This spell adds an extreme deal of extra protection to a piece of armour.  The enchantment "
"limit for this spell is higher than that of master fortification.  "
...
"***This is a very powerful spell and only the most dedicated enchanters may use it. A skill of 210% is needed to cast this spell.\n"
...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  if((int)caster->query_skill("enchantment") < 210 ) {
  ...
  if((int)at->query_property("enchantment") >= 100) {
    message("info", "This armour may receive no further extreme "+
      "enchantments.", caster);
    caster->add_mp(props["mp cost"]);
    remove();
    return;
...
//ADD
if(power >= 6) {
    at->set_property("enchantment", 100);
	} 
//END
