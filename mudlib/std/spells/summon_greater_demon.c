...
void create() {
    ::create();
    set_property("name","summon greater demon");
    set_property("skill","chaos magic");
    set_property("casting time",59);
    set_property("base mp cost",82);
    set_property("dev cost",71);
    set_property("fast dev cost",213);
    set_property("spell level", 14);
    set_property("moon","luna");
    set_property("caster message","You summon a Greater Demon.");
    set_property("target message","");
    set_property("observer message","$C summons a greater demon.");
...
void info() {
message("help",
"This spell is used to summon a greater demon.  You must make a successful "
"control roll at -30 ('help skill control demon'), or he may simply disappear, "
...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  seteuid(getuid());
  ob = new("/std/diewarzau/obj/pet/greater_demon");
  ...
  roll = random(100);
  if(roll < (int)caster->query_skill("control demon")-30) {
    message("info", "The demon is successfully contolled.", caster);
    ob->set_owner((string)caster->query_name());
    remove();
    return;
  } else if(roll - ((int)caster->query_skill("control demon")-30) < 15) {
    ...
    remove();
    return;
  } else if(roll - ((int)caster->query_skill("control demon")-30) < 30) {
