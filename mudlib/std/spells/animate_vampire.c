...
void create() {
    ::create();
    set_property("name","animate vampire");
    set_property("skill","necromancy");
    set_property("duration", "permanent");
    set_property("casting time",4);
    set_property("base mp cost",109);
    ...
    set_property("prereq", "animate ghast");
...
void info() {
message("help",
"This spell animates a corpse into a vampire who will serve you as an undead thrall.  "
...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  set_work_message("%^GREEN%^You mold the corpse.");
    time = (1900+((190-(int)caster->query_skill("necromancy"))*power));
    if(archp(caster)) {
	time = 1;
	}
...
void finish_work(object caster, object at, int power) {
  ...
  at->remove();
  seteuid(getuid());
  ob = new("/std/spells/summon/vampire");
  ob->set_up(power);
