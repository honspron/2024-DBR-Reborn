...
void spell_func(object caster, object at, int power, string args, int flag) {
    ...
    if(caster->query_base_skill("attack") > 0) ob->add_new_skill_bonus("attack", 30*bonus);
	if(caster->query_base_skill("attack") < 1) ob->add_skill("attack", 30*bonus);
    if(caster->query_base_skill("two handed staff") > 0) ob->add_new_skill_bonus("two handed staff", 30*bonus);
	if(caster->query_base_skill("two handed staff") < 1) ob->add_skill("two handed staff", 30*bonus);
    }
	...
	if(caster->query_base_skill("black magic") > 0) ob->add_new_skill_bonus("black magic", 60*bonus);
	if(caster->query_base_skill("black magic") < 1) ob->add_skill("black magic", 60*bonus);
    }
    if(args == "revenant") {
	...
	if(caster->query_base_skill("attack") > 0) ob->add_new_skill_bonus("attack", 50*bonus);
	if(caster->query_base_skill("attack") < 1) ob->add_skill("attack", 50*bonus);
    if(caster->query_base_skill("two handed staff") > 0) ob->add_new_skill_bonus("two handed staff", 50*bonus);
	if(caster->query_base_skill("two handed staff") < 1) ob->add_skill("two handed staff", 50*bonus);
    }
    if(args == "blood hound") {
	...
	if(caster->query_base_skill("body alteration") > 0) ob->add_new_skill_bonus("body alteration", 80*bonus);
	if(caster->query_base_skill("body alteration") < 1) ob->add_skill("body alteration", 80*bonus);
    }
    if(args == "bone hound") {
	...
	if(caster->query_base_skill("attack") > 0) ob->add_new_skill_bonus("attack", 80*bonus);
	if(caster->query_base_skill("attack") < 1) ob->add_skill("attack", 80*bonus);
    if(caster->query_base_skill("two handed staff") > 0) ob->add_new_skill_bonus("two handed staff", 80*bonus);
	if(caster->query_base_skill("two handed staff") < 1) ob->add_skill("two handed staff", 80*bonus);
    }
    if(args == "ghost") {
    ...
    if(caster->query_base_skill("black magic") > 0) ob->add_new_skill_bonus("black magic", 110*bonus);
	if(caster->query_base_skill("black magic") < 1) ob->add_skill("black magic", 110*bonus);
    if(caster->query_base_skill("owedquar's will") > 0) ob->add_new_skill_bonus("owedquar's will", 110*bonus);
	if(caster->query_base_skill("owedquar's will") < 1) ob->add_skill("owedquar's will", 110*bonus);
    }
    if(args == "ghast dog") {
	...
	if(caster->query_base_skill("parry") > 0) ob->add_new_skill_bonus("parry", 75*bonus);
	if(caster->query_base_skill("parry") < 1) ob->add_skill("parry", 75*bonus);
    if(caster->query_base_skill("dodge") > 0) ob->add_new_skill_bonus("dodge", 75*bonus);
	if(caster->query_base_skill("dodge") < 1) ob->add_skill("dodge", 75*bonus);
    }
	...
	if(caster->query_base_skill("attack") > 0) ob->add_new_skill_bonus("attack", 125*bonus);
	if(caster->query_base_skill("attack") < 1) ob->add_skill("attack", 125*bonus);
    if(caster->query_base_skill("two handed staff") > 0) ob->add_new_skill_bonus("two handed staff", 125*bonus);
	if(caster->query_base_skill("two handed staff") < 1) ob->add_skill("two handed staff", 125*bonus);
    }
