...
void create() {
    ::create();
    set_property("name","animate abomination");
    set_property("skill","necromancy");
    set_property("duration", "permanent");
    set_property("casting time",4);
    set_property("base mp cost",266);
    set_property("dev cost", 149);
    ...
    set_property("prereq", "animate dracolich");
...
void info() {
message("help",
"This spell animates a corpse into a Unholy Abomination, a powerful, undead thing, who will serve you as an undead thrall.  "
...
"time to cast.  The spell must be cast on an embalmed corpse of level 30 or higher, which must "
...
time = (10500+((660-(int)caster->query_skill("necromancy"))*power));
if(archp(caster)) {
time = 1;
}
...
void finish_work(object caster, object at, int power) {
  ...
  message("info", "The corpse transforms into a "+
	  "Unholy Abomination.", caster);
  message("info", (string)caster->query_cap_name() + " utters some "+
	  "necromantic incantations, and an embalmed corpse "+
	  "transforms into a Unholy Abomination.",
	  environment(caster), ({ caster }) );
	     	if(archp(caster)) {
	    }else{  
	   message("info", "%^GREEN%^%^BOLD%^The world shakes and a Unholy Abomination is born!",
	   users()); }
  at->remove();
  seteuid(getuid());
  ob = new("/std/spells/summon/abomination");
  ob->set_up(power);
