...
void create() {
    ::create();
    set_property("name","mastered weapon");
    ...
    set_property("caster message","You begin to master the weapon.");
    set_property("target message","");
    set_property("observer message","$C begins to enchant a weapon.");
    set_property("spell type",({ }));
    set_property("target type", "any");
    set_property("must be present", 1);
    set_property("prereq", "claimed weapon");
    return;
}

void info() {
message("help",
"This is a very powerful enchantment and the stronger version of claimed weapon. When cast on a weapon, the weapon is bound to the caster.  "
...
"wields the weapon, however, he/she will have great skill with the weapon "
...
//  int ench;
  ...
  message("info", (string)caster->query_cap_name() + " utters some "+
	  ...
	  environment(caster), ({ caster }) );
	at->add_enchantment("mastered", "master activate", power,
	  3600 / (power/2+1), 0);
	if(strsrch((string)at->query_type(), "two handed") > -1)
	  at->set_type("two handed "+(string)caster->query_name()+" mastered");
	else
	  at->set_type((string)caster->query_name()+" mastered");
	message("info", "See the help on this spell to see how to "
	  "activate the mastered weapon.", caster);
	at->set_property("mastered by", (string)caster->query_name());
	extra = at->query_property("extra long");
	if(!extra) extra = ({});
	else if(stringp(extra)) extra = ({ extra });
	else if(!pointerp(extra)) extra = ({});
	extra += ({ "%^CYAN%^This weapon is mastered by %^RESET%^%^BOLD%^"+
