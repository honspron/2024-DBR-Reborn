...
set_short("[1]ferocious silver dragon");
...
a = (random(100));
if(a<10) {
tell_room(environment(this_object()), "%^YELLOW%^The Silver Dragon plants its feet into the ground and begins to charge a powerful burst of flame.%^RESET%^");
force_me("cast *6 violent blaze");
}
else
return;
}
