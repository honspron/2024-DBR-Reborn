...
int query_invis() { 
  if(environment(who_shadowed)->query_terrain()) return 1;
  else return 0;
 }
...
string query_cap_name() { 
  ...
  if(base_name(ob) == "/cmds/mortal/_who") return (string)next_shadow()->query_cap_name();
  if(environment(who_shadowed)->query_terrain()) return "Someone";
