#define DAMAGE_STR ({ "no", "slight", "little", "moderate", "considerable", \
			"much", "severe", "grave" })
#include <security.h>
...
void warn_expire(object me, int to_go) {
  ...
  message("info", "%^RED%^%^BOLD%^Holy mission expires in "+
	  sprintf("%d min., %d sec.  %sYou have taken %s damage.",
		  to_go/60, to_go%60, "%^RESET%^", DAMAGE_STR[idx]), 
          who_shadowed);
  to_go -= 20;
  if(to_go > 1)
...
int do_damage(string limb, int amt) {
  ...
  if(member_array(limb, (string *)who_shadowed->query_limbs()) < 0)
    return 0;
  res = (int)who_shadowed->query_max_dam(limb)*2 -
	(int)who_shadowed->query_dam(limb);
  if(amt > res) amt = res;
  damage += amt;
  return amt;
}
...
mixed query(string str){
    if(str == "no quit") return 1;
    return next_shadow()->query(str);
}
