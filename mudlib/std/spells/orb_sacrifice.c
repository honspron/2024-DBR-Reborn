...
void info() {
    message("help", "This magic gives a Nethermancer the means of sacrificing a soul orb by drawing power out of a nether category two soul trapped inside of its design and utilizing it as significantly strong knowledge for either skills or spells. After a sacrifice is complete on a given orb, the power gained will last until the caster logs out from the mud. This magic will only succeed if the orb it is cast upon is finalized with the spell 'orb finalization', and each sacrifice per login must be for a different type of soul for stacking is prohibited. The soul orb will shatter once this spell succeeds from the soul being outwardly drawn and the orb itself is gone forever. Below is a listing of all sacrifices that are valid with this spell alongside their level requirements and the skills/spells that are attained."+
      "\n\nBlood Fiend (Level 20)             Consume Soul, +105% spiritmancy\nVampiric Hound (Level 21)          Greater Curse, +115% curse lore\nFanged Spirit (Level 23)           +100% parry, +100% dodge\nBlack Essence (Level 25)           +475 AC Boost\nRed Ghoul (Level 27)               +150% attack, +150% two handed staff\nDark Knight (Level 30)             +3 Perfect Armour Bonus\nBlood Knight (Level 32)            Devour Soul, +140% spiritmancy\nEssence Warrior (Level 33)         +150% perception, +150% reverse stroke,\n                                   +75% combat awareness\n\nSyntax: 'cast *6 orb sacrifice at orb with blood knight'", this_player());
}
...
void spell_func(object caster, object at, int power, string args, int flag) {
	...
	ob->add_spell("lifesteal", power);
	if(caster->query_base_skill("black magic") > 0) ob->add_new_skill_bonus("black magic", 105*bonus);
	if(caster->query_base_skill("black magic") < 1) ob->add_skill("black magic", 105*bonus);
    }
	...
	if(caster->query_base_skill("curse lore") > 0) ob->add_new_skill_bonus("curse lore", 115*bonus);
	if(caster->query_base_skill("curse lore") < 1) ob->add_skill("curse lore", 115*bonus);
    }
	...
	if(caster->query_base_skill("attack") > 0) ob->add_new_skill_bonus("attack", 150*bonus);
	if(caster->query_base_skill("attack") < 1) ob->add_skill("attack", 150*bonus);
	if(caster->query_base_skill("two handed staff") > 0) ob->add_new_skill_bonus("two handed staff", 150*bonus);
	if(caster->query_base_skill("two handed staff") < 1) ob->add_skill("two handed staff", 150*bonus);
    }
    if(args == "dark knight") {
	...
	if(caster->query_base_skill("perception") > 0) ob->add_new_skill_bonus("perception", 150*bonus);
	if(caster->query_base_skill("perception") < 1) ob->add_skill("perception", 150*bonus);
	if(caster->query_base_skill("reverse stroke") > 0) ob->add_new_skill_bonus("reverse stroke", 150*bonus);
	if(caster->query_base_skill("reverse stroke") < 1) ob->add_skill("reverse stroke", 150*bonus);
	if(caster->query_base_skill("combat awareness") > 0) ob->add_new_skill_bonus("combat awareness", 75*bonus);
	if(caster->query_base_skill("combat awareness") < 1) ob->add_skill("combat awareness", 75*bonus);
    }
