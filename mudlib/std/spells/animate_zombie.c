...
void create() {
    ::create();
    set_property("name","animate zombie");
    set_property("skill","necromancy");
    set_property("duration", "permanent");
    set_property("casting time",4);
    set_property("base mp cost",46);
    set_property("dev cost", 16);
    set_property("fast dev cost", 48);
    ...
    set_property("prereq", "animate skeleton");
...
void info() {
message("help",
"This spell animates a corpse into a zombie who will serve you as an undead thrall.  "
...
int time;
  ...
  set_work_message("%^GREEN%^You mold the corpse.");
     time = ((630 - (int)caster->query_skill("necromancy")) +(100*power));
    if(archp(caster)) {
	time = 1;
	}
...
void finish_work(object caster, object at, int power) {
  ...
  message("info", (string)caster->query_cap_name() + " utters some "+
	  ...
	  "transforms into a hideous zombie.",
	  environment(caster), ({ caster }) );
  at->remove();
  seteuid(getuid());
  ob = new("/std/spells/summon/zombie");
  ob->set_up(power);
