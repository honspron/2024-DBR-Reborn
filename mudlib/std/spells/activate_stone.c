...
void create() {
    ::create();
set_property("name","activate stone");
...
"time to cast. If you are interrutpted while casting, you may pick up again "
"at a later time.",
this_player());
}
...
start_work(at,caster,(time*mod)/caster->query_skill("alchemy"),power);
  return;
}
