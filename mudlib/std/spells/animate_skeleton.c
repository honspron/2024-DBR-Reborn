...
void create() {
    ::create();
    set_property("name","animate skeleton");
    ...
    set_property("caster message","You begin to animate the corpse, stripping it of flesh.");
...
void info() {
message("help",
"This spell animates a corpse into a skeleton who will serve you as an undead thrall.  "
...
void finish_work(object caster, object at, int power) {
  ...
  message("info", (string)caster->query_cap_name() + " utters some "+
	  ...
	  "transforms into an animated skeleton.",
	  environment(caster), ({ caster }) );
  at->remove();
  seteuid(getuid());
