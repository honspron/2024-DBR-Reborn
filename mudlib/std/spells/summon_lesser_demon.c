...
void create() {
    ::create();
    set_property("name","summon lesser demon");
    ...
    set_property("caster message","You summon a Lesser Demon.");
    set_property("target message","");
    set_property("observer message","$C summons a lesser demon.");
...
void info() {
message("help",
"This spell is used to summon a lesser demon.  You must make a successful "
"control roll ('help skill control demon'), or he may simply disappear, "
...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  seteuid(getuid());
  ob = new("/wizards/diewarzau/obj/pet/lesser_demon");
  ...
  roll = random(100);
  if(roll < (int)caster->query_skill("control demon")) {
    message("info", "The demon is successfully contolled.", caster);
    ob->set_owner((string)caster->query_name());
    remove();
    return;
  } else if(roll - (int)caster->query_skill("control demon") < 15) {
    ...
    remove();
    return;
  } else if(roll - (int)caster->query_skill("control demon") < 30) {
