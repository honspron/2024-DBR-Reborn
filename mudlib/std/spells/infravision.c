...
void create() {
    ...
    set_property("caster message","You cast infravision at $T.");
    set_property("target message","$C casts infravision at you.");
    set_property("observer message","$C casts infravision at $T.");
...
void spell_func(object caster, object at, int power, string args, int flag)
{
  ...
  seteuid(getuid());
  if(flag) {
    message("info", "ACK!  You accidentally blind "+ (string)at->query_cap_name() + "!", caster);
    message("info", (string)caster->query_cap_name() +" accidentally BLINDS you!", at);
