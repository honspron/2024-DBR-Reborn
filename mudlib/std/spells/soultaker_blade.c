...
void info() {
message("help", "This spell must be cast on a weapon which the caster is holding.  It imbues "
...
//  object ob;
  ...
  if(at->query("vampiric")) {
    message("info", "A weapon may only receive one spell of this type at a time.", caster);
    caster->add_mp(props["mp cost"]);
    remove();
    return;
  }
...
//TLNY2022 fixed who ever did this shame on them
/*
  if(flag)
