...
void create() {
    ...
    set_property("prereq", "black edge");
return;
}
...
lvl = (caster->query_level()/2);    
    ...
    message("info", "%^B_RED%^%^BOLD%^%^BLACK%^You imbue the weapon with mortifying evil power!%^RESET%^", caster);
        if(power < 4 ){
ob->set_extra_wc(([ "unholy" : 14*power+lvl, "aether" : 14*power+lvl, "disruption" : 14*power+lvl   ]));
         ob->start_shadow(at, props["duration"], "%^RED%^An abyss blade spell wears off.%^RESET%^");
        }
        else{
ob->set_extra_wc(([ "unholy" : 24*power+lvl, "aether" : 24*power+lvl, "disruption" : 24*power+lvl   ]));
         ob->start_shadow(at, props["duration"], "%^RED%^An abyss blade spell wears off.%^RESET%^");
        }
    }
    else{
