...
// HONSPRON 2021 - Cant cast if resting or prone
    if(caster->query_state() == "rest") {
	message("info", "You can't cast spells while resting.",
	  caster);
	return;
    }
    if(caster->query_state() == "prone") {
	message("info", "You can't cast spells cause you got knocked on your butt.",
	  caster);
	return;
    }
//TLNY2021 ADD Boss room prevent casting for all players
    if(environment(caster)->query_property("no boss spell") && ((string)this_player()->query_race() != "beholder") ) {
	message("info","The Beholders anti magic field prevents you from casting.",caster);
	remove();
	return;
    }
//END
...
// Ritual fumbling
	...
	} else if(roll > (skill+100) && !props["ritual final"]) {
// Ritual sputtering
	if(props["ritual dud"] || props["ritual spell"]){
