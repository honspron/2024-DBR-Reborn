...
void create() {
    ::create();
    set_property("name","animate demilich");
    set_property("skill","necromancy");
    set_property("duration", "permanent");
    set_property("casting time",4);
    set_property("base mp cost",176);
    set_property("dev cost", 97);
    set_property("fast dev cost", 301);
    ...
    set_property("prereq", "animate lich");
...
void info() {
message("help",
"This spell animates a corpse into a Demi-lich, a powerful, spellcasting undead servant, who will serve you as an undead thrall.  "
...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  set_work_message("%^GREEN%^You mold the corpse.");
    time = (4000+((400-(int)caster->query_skill("necromancy"))*power));
    if(archp(caster)) {
	time = 1;
	}
...
void finish_work(object caster, object at, int power) {
  ...
  message("info", (string)caster->query_cap_name() + " utters some "+
	  ...
	  "transforms into a Demi-Lich.",
	  environment(caster), ({ caster }) );
  at->remove();
  seteuid(getuid());
  ob = new("/std/spells/summon/demilich");
  ob->set_up(power);
