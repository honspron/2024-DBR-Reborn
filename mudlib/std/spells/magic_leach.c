...
void create() {
    ...
    set_property("caster message","You leach magic power from the rest of the mortals.");
...
void info() {
message("help",
"This extremely powerful spell leaches magical power "
...
void spell_func(object caster, object at, int power, string args, int flag)
{
  ...
  foreach(ob in inv) {
    ...
    if((random(100) >= (int)ob->query_skill("spell sense")) &&
      ...
      continue;
    message("info", ""
            "You sense "+CQN+" draining mana from you with a magic leach!",
 
            ob);
    if(random(100) >= (2*(int)ob->query_skill("spell sense")/3))
      continue;
    message("info", "You sense "+possessive(caster)+
