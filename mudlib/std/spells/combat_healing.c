...
void create() {
    ...
    set_property("stack key", "com_heal");
...
void info() {
    message("help",
      ...
      "automatically healed of up to 20 hp per power level (5d4).  The spell expires "
      "when the heal 'battery' is exhausted.  The battery contains 60 hp per power level.  "
...
void spell_func(object caster, object at, int power, string args, int flag) {
    ...
    at->set("com_heal #", (int)at->query("com_heal #") + 1);
    call_out("remove_stack", props["duration"], at);
    ob = new("/std/spells/shadows/auto_heal_shad");
    ob->set_perc(perc);
    ob->set_pow(power);
    ob->start_shadow(at, 60*power, "%^CYAN%^The heal battery has expired.");
    remove();
    return;
}
