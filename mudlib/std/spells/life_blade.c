...
void create() {
    ...
    set_property("caster message","You cast life blade on $T!");
    set_property("target message","");
    set_property("observer message","$C casts life blade on $T.");
...
"the caster.\n"
"Usage: cast *2 life blade at sword",
this_player());
}
...
//  object ob;
  ...
  if(at->query_hit()) {
    message("info", "This weapon may not recieve life blade at "
            "this time.", caster);
    caster->add_mp(props["mp cost"]);
    remove();
    return;
...
void remove_spell(object at, object caster) {
  ...
  at->set_hit(0);
  at->set("vampiric", 0);
  if(caster) message("info", "%^MAGENTA%^%^BOLD%^Life blade wears off.",
		     caster);
  remove();
  return;
}
