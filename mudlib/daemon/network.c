...
/*No don't because I won't rebuild it by hand again :)
...
/*
if (find_player("exash")) tell_object(find_player("exash"), "Outgoing: "+msg+"\n");
*/
...
void read_callback(int sock, string msg, string host) { 
    ...
    if(!msg) return; 
if (find_player("exash")) tell_object(find_player("exash"), msg+"\n");
    tmp + ""; 
if (bufferp(msg)) {
msg = read_buffer(msg);
}
...
int mud_exists(string mud) { 
if (!__Muds) {
if (find_player("exash")) tell_object(find_player("exash"), "Error when looking up MUD "+mud+"\n");
return 1;
}
