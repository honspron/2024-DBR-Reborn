...
void aether_A_crit(object from, object to) {
  do_critical(from, to, ({ "aether A" }) );
  return;
}

void aether_E_crit(object from, object to) {
  do_critical(from, to, ({ "aether E" }) );
  return;
}

void aether_crit(object from, object to) {
  string crit;

  crit = sprintf("aether %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}

void infernal_A_crit(object from, object to) {
  do_critical(from, to, ({ "infernal A" }) );
  return;
}

void infernal_E_crit(object from, object to) {
  do_critical(from, to, ({ "infernal E" }) );
  return;
}

void infernal_crit(object from, object to) {
  string crit;

  crit = sprintf("infernal %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}

void unholy_A_crit(object from, object to) {
  do_critical(from, to, ({ "unholy A" }) );
  return;
}

void unholy_E_crit(object from, object to) {
  do_critical(from, to, ({ "unholy E" }) );
  return;
}

void unholy_crit(object from, object to) {
  string crit;

  crit = sprintf("unholy %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}

void holy_A_crit(object from, object to) {
  do_critical(from, to, ({ "holy A" }) );
  return;
}

void holy_E_crit(object from, object to) {
  do_critical(from, to, ({ "holy E" }) );
  return;
}

void holy_crit(object from, object to) {
  string crit;

  crit = sprintf("holy %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}

void plasma_A_crit(object from, object to) {
  do_critical(from, to, ({ "plasma A" }) );
  return;
}

void plasma_E_crit(object from, object to) {
  do_critical(from, to, ({ "plasma E" }) );
  return;
}
...
void disruption_A_crit(object from, object to) {
  do_critical(from, to, ({ "disruption A" }) );
  return;
}

void disruption_E_crit(object from, object to) {
  do_critical(from, to, ({ "disruption E" }) );
  return;
}

void disruption_crit(object from, object to) {
  string crit;
...
void electricity_E_crit(object from, object to) {
  do_critical(from, to, ({ "electricity E" }) );
  return;
}
...
void vacuum_A_crit(object from, object to) {
  do_critical(from, to, ({ "vacuum A" }) );
  return;
}

void vacuum_E_crit(object from, object to) {
  do_critical(from, to, ({ "vacuum E" }) );
  return;
}
...
void cold_A_crit(object from, object to) {
  do_critical(from, to, ({ "cold A" }) );
  return;
}
...
void cold_crit(object from, object to) {
  string crit;

  crit = sprintf("cold %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}
...
void stress_A_crit(object from, object to) {
  do_critical(from, to, ({ "stress A" }) );
  return;
}

void stress_E_crit(object from, object to) {
  do_critical(from, to, ({ "stress E" }) );
  return;
}

void stress_crit(object from, object to) {
  string crit;

  crit = sprintf("stress %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}
...
void impact_E_crit(object from, object to) {
  do_critical(from, to, ({ "impact E" }) );
  return;
}
...
void strike_E_crit(object from, object to) {
  do_critical(from, to, ({ "strike E" }) );
  return;
}

void strike_crit(object from, object to) {
  string crit;

  crit = sprintf("strike %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}

void impaling_A_crit(object from, object to) {
  do_critical(from, to, ({ "impaling A" }) );
  return;
}

void impaling_E_crit(object from, object to) {
  do_critical(from, to, ({ "impaling E" }) );
  return;
}

void impaling_crit(object from, object to) {
  string crit;

  crit = sprintf("impaling %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}

void cutting_A_crit(object from, object to) {
  do_critical(from, to, ({ "cutting A" }) );
  return;
}

void cutting_E_crit(object from, object to) {
  do_critical(from, to, ({ "cutting E" }) );
  return;
}

void cutting_crit(object from, object to) {
  string crit;

  crit = sprintf("cutting %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}

void crushing_A_crit(object from, object to) {
  do_critical(from, to, ({ "crushing A" }) );
  return;
}

void crushing_E_crit(object from, object to) {
  do_critical(from, to, ({ "crushing E" }) );
  return;
}

void crushing_crit(object from, object to) {
  string crit;

  crit = sprintf("crushing %s", ({ "B", "C", "D" })[random(3)]);
  do_critical(from, to, ({ crit }) );
  return;
}
...
void extra_attack(object me, object you) {
  message("info","%^YELLOW%^You attack again!%^RESET%^", (me));
  me->continue_attack();
  return;
}
...
void increase_ac_cold(object from, object to) {

  object ob;
  message("info","%^BOLD%^%^BLUE%^A ice shield forms around you granting protection%^RESET%^", (from));
  seteuid(getuid());
  ob = new("/std/spells/shadows/ac_shadow");
  ob->set_shad_ac(10 + random(10));
  ob->set_expire_message("%^BOLD%^%^BLUE%^The ice shield melts%^RESET%^");
  ob->start_shadow(from, 40);
  return;
}
...
void increase_str(object from, object to) {
  ...
  object ob;
  message("info","%^YELLOW%^Your Strength is inscreased temporarily%^RESET%^", (from));
  seteuid(getuid());
  ob = new("/std/spells/shadows/stat_shadow_new");
  ob->set_stat("strength");
  ob->set_mod(1 + random(10));
  ob->set_expire_message("%^YELLOW%^Your Strength Fades.%^RESET%^");
  ob->start_shadow(from, 40);
  return;
}

void increase_con(object from, object to) {

  object ob;
  message("info","%^YELLOW%^Your Constitution is inscreased temporarily%^RESET%^", (from));
  seteuid(getuid());
  ob = new("/std/spells/shadows/stat_shadow_new");
  ob->set_stat("constitution");
  ob->set_mod(1 + random(10));
  ob->set_expire_message("%^YELLOW%^Your Constitution Fades.%^RESET%^");
  ob->start_shadow(from, 50);
  return;
}

void increase_dex(object from, object to) {

  object ob;
  message("info","%^YELLOW%^Your Dexterity is inscreased temporarily%^RESET%^", (from));
  seteuid(getuid());
  ob = new("/std/spells/shadows/stat_shadow_new");
  ob->set_stat("dexterity");
  ob->set_mod(1 + random(10));
  ob->set_expire_message("%^YELLOW%^Your Dexterity Fades.%^RESET%^");
  ob->start_shadow(from, 40);
  return;
}

void increase_cha(object from, object to) {

  object ob;
  message("info","%^YELLOW%^Your Charisma is inscreased temporarily%^RESET%^", (from));
  seteuid(getuid());
  ob = new("/std/spells/shadows/stat_shadow_new");
  ob->set_stat("charisma");
  ob->set_mod(1 + random(10));
  ob->set_expire_message("%^YELLOW%^Your Charisma Fades.%^RESET%^");
  ob->start_shadow(from, 60);
  return;
}

void increase_int(object from, object to) {

  object ob;
  message("info","%^YELLOW%^Your Intelligence is inscreased temporarily%^RESET%^", (from));
  seteuid(getuid());
  ob = new("/std/spells/shadows/stat_shadow_new");
  ob->set_stat("intelligence");
  ob->set_mod(1 + random(10));
  ob->set_expire_message("%^YELLOW%^Your Intelligence Fades.%^RESET%^");
  ob->start_shadow(from, 60);
  return;
}

void increase_wis(object from, object to) {

  object ob;
  message("info","%^YELLOW%^Your Wisdom is inscreased temporarily%^RESET%^", (from));
  seteuid(getuid());
  ob = new("/std/spells/shadows/stat_shadow_new");
  ob->set_stat("wisdom");
  ob->set_mod(1 + random(10));
  ob->set_expire_message("%^YELLOW%^Your Wisdom Fades.%^RESET%^");
  ob->start_shadow(from, 60);
  return;
}
...
void remove_pet(object ob) {
	message("info",(string)ob->query_short()+" suddenly dissapears.", environment(ob));
	ob->remove();
}
