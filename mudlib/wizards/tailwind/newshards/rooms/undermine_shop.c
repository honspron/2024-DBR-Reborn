...
//    -Diewarzau 1/29/96
//    DarkeLIB 1.0
#define MINE_D "/daemon/undermine_d"
#define MAX_SOUTH 39
#define MAX_EAST 79
#include "../newshards.h"
#include <rooms.h>
...
void create() {
  ::create();
  add_exit(ROOMS+"street5", "east");
  set_short("The UnderMine Shop");
  set_long(
  "This small shack is the UnderMine shop.  Here you may buy a mine somewhere "
  "on the underworld map and hire workers for it.  Mining tools lie about and "
...
"WARNING: Undermines do not produce at the rate of their surface\n"
"cousins. Expect a smaller rate of return on your investment!\n"
"\n"
...
void pick_plot(string coords, object tp) {
  ...
  room = find_object_or_load(sprintf("/d/damned/virtual/room_%d_%d.underworld", east, south));
  if(!room) {
    message("info", "Those coordinates are off the map!", tp);
    printf("coordinates> ");
    input_to("pick_plot", 0, tp);
  ...
  write_file("/d/damned/data/underworld_items.db",
      sprintf("%d_%d:/d/damned/virtual/%s_mine1_%d_%d.undermine:mine:The entrance to %s's Mine\n",
  ...
  tp->set_property("has undermine", sprintf("/d/damned/virtual/%s_mine2_%d_%d.undermine",
                                  (string)tp->query_name(), east, south));
  tp->save_player((string)tp->query_name());
  MINE_D->register_mine(sprintf("/d/damned/virtual/%s_mine2_%d_%d.undermine",
                                  (string)tp->query_name(), east, south));
...
string has_mine(object who) {
  ...
  if(!(file = (string)who->query_property("has undermine")))
    return 0;
  if(sscanf(file, "/d/damned/virtual/%*s_mine2_%d_%d.undermine", x, y) != 3) {
    who->remove_property("has mine");
    return 0;
  }
room = find_object_or_load(sprintf("/d/damned/virtual/room_%d_%d.underworld", x, y));
  if((string)room->query_exit("mine") != sprintf("/d/damned/virtual/%s_mine1_%d_%d.undermine",
            (string)who->query_name(), x, y)) {
    who->remove_property("has undermine");
    return 0;
  }
  return file;
}
